...
class FoodChoiceApplicationTests {
    ...
    void getRestaurantOverviewListTest1() throws Exception{
        ...
        result.andDo(print())
                ...
                .andExpect(jsonPath("$.response[0].info").value(equalTo("압구정역 2102m")))
                .andExpect(jsonPath("$.response[1].name").value(equalTo("요거멜리아")))
                .andExpect(jsonPath("$.response[1].info").value(equalTo("압구정역 2155m")))
                .andExpect(jsonPath("$.response[2].name").value(equalTo("팔구사(894)@")))
                .andExpect(jsonPath("$.response[2].info").value(equalTo("압구정역 3363m")))
        ;
    }
    ...
    public void findNearRestaurantTest1() throws NotFoundKeywordException {
        List<RestaurantOverviewDto> restaurantOverviewList = foodManagementService.findNearRestaurant("미아역",0,3);
    ...
    public void findNearRestaurantTest2() throws NotFoundKeywordException {
        List<RestaurantOverviewDto> restaurantOverviewList = foodManagementService.findNearRestaurant("미아역",3,3);
    ...
    public void findNearRestaurantTest4() throws NotFoundException {
        List<RestaurantOverviewDto> restaurantOverviewList = foodManagementService.findNearRestaurant("미아역", 99999999, 0);
    ...
    public void findNearRestaurantTest5() throws NotFoundException {
        List<RestaurantOverviewDto> restaurantOverviewList = foodManagementService.findNearRestaurant("미아역", 0, 0);
    ...
    public void getDescOfRestaurant() throws Exception {
        ResultActions result = mockMvc.perform(
                get("/api/JSON/restaurant/getDescription")
                        .accept(MediaType.APPLICATION_JSON)
                        .param("restaurantId", "14")
        );
        ...
        result.andDo(print())
                .andExpect(handler().handlerType(RestaurantController.class))
                .andExpect(handler().methodName("getDescription"))
                .andExpect(jsonPath("$.success", is(true)))
                .andExpect(jsonPath("$.response").exists())
                .andExpect(jsonPath("$.response[0].name", is("떡만두국")))
                .andExpect(jsonPath("$.response[0].price", is(7000)))
